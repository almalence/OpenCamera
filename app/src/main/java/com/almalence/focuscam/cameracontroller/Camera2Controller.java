...
/* <!-- +++
 package com.almalence.focuscam_plus.cameracontroller;
 +++ --> */
// <!-- -+-
...
public class Camera2Controller
{
	...
	private static boolean				burstCaptureFocusBracketing = false;
...
//		Log.e(TAG, "fillPictureSizeList. USE captureFormat.");
	...
	{
		Log.e(TAG, "LENS_INFO_FOCUS_DISTANCE_CALIBRATION = " + Camera2Controller.getInstance().camCharacter.get(CameraCharacteristics.LENS_INFO_FOCUS_DISTANCE_CALIBRATION));
		Log.e(TAG, "LENS_INFO_MINIMUM_FOCUS_DISTANCE = " + Camera2Controller.getInstance().camCharacter.get(CameraCharacteristics.LENS_INFO_MINIMUM_FOCUS_DISTANCE));
		Log.e(TAG, "LENS_INFO_HYPERFOCAL_DISTANCE = " + Camera2Controller.getInstance().camCharacter.get(CameraCharacteristics.LENS_INFO_HYPERFOCAL_DISTANCE));
...
//		final int previewHeight = ApplicationScreen.getPreviewSurfaceView().getHeight();
		final int previewWidth  = ApplicationScreen.getPreviewTextureView().getWidth();
		final int previewHeight = ApplicationScreen.getPreviewTextureView().getHeight();
...
//		stillRequestBuilder = Camera2Controller.getInstance().camDevice.createCaptureRequest(CameraDevice.TEMPLATE_MANUAL);
...
//        precaptureRequestBuilder = Camera2Controller.getInstance().camDevice.createCaptureRequest(CameraDevice.TEMPLATE_MANUAL);
...
//			else
	...
	private static void SetupPerFrameParameters(int ev, int gain, long expo, float focusDistance, boolean isRAWCapture)
	{
		...
		if(focusDistance != -1)
		{
			Log.e(TAG,"stillRequestBuilder set LENS_FOCUS_DISTANCE " + focusDistance);
			stillRequestBuilder.set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_OFF);
		...
		//						if (evRequested == null) requestID = tmp;
			...
			else
			{
				...
				for (int n = 0; n < nFrames; ++n)
				{
                    ...
                    CaptureRequest request = stillRequestBuilder.build();
//                    float dist = request.get(CaptureRequest.LENS_FOCUS_DISTANCE);
//                    Log.e(TAG, "REQUEST DISTANCE = " + dist);
...
//			CreateRequests(format);
	...
	private static void startFocusBracketingCapture(final float[] focusDistances, final List<CaptureRequest> requests)
	{
		...
		burstCaptureFocusBracketing = prefs.getBoolean(ApplicationScreen.getAppResources().getString(R.string.Preference_FocusBracketingBurstPref), false);


		if(burstCaptureFocusBracketing)
		{
			if (Camera2Controller.getInstance().mCaptureSession != null) {
				try {
					int requestID = Camera2Controller.getInstance().mCaptureSession.captureBurst(requests, stillCaptureCallback, null);

					pluginManager.addRequestID(0, requestID);

//				for (int n = 0; n < requests.size(); ++n)
//				{
//					Camera2Controller.getInstance().mCaptureSession.capture(requests.get(n), stillCaptureCallback, null);
//
//				}
				} catch (CameraAccessException e) {
					e.printStackTrace();
...
//				}
	...
	public final static CameraDevice.StateCallback openCallback = new CameraDevice.StateCallback()
	{
		...
		public void onOpened(CameraDevice arg0)
		{
			...
			ApplicationScreen.instance.configureCamera(false);
//			messageHandler.sendEmptyMessage(ApplicationInterface.MSG_CAMERA_OPENED); //TODO: commented for TextureView case. Originally remove configureCamera call!
		}
...
//					captureAllowed = true;
			...
			try {
				...
				if(fbRequested && !burstCaptureFocusBracketing && captureAllowed)
				{
					...
					{
						Log.e(TAG, "Diff less than 0.1f. Capture!");
						if(Camera2Controller.getInstance().mCaptureSession != null)
						{
							try
							{
	...
	public final static ImageReader.OnImageAvailableListener imageAvailableListener = new ImageReader.OnImageAvailableListener()
	{
		...
		public void onImageAvailable(ImageReader ir)
		{
			...
			} else
			{
				...
				if(focusDistance != null && currentFrameIndex < totalFrames && !burstCaptureFocusBracketing)
				{
