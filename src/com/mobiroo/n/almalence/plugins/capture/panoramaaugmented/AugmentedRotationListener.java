...
{		
	...
	public void calculateAccMagOrientation()
	{
    	if (this.accMagOrientation == null)
    	{
			...
			this.gyroOrientation[0] = 1;	// give some initial direction to get meaningful results from fake magneto
		...
		// LP-filter data from accelerometer (nearly-discard data from non-stationary samplings)
		filterGravity(this.data_acceleration, gravity);
		
		// generate fake magneto readings from gyroscope (invert gyro matrix to get conversion from world coordinates to device coordinates)
		...
		fakeMagneto[0] = fakeMagnetoMatrix[3];	// compose vector of magnetic field 
		fakeMagneto[1] = fakeMagnetoMatrix[4];	// using axis pointing to north (0,1,0)
	...
	public void magnetoFunction(final SensorEvent event)
	{
		...
		float norm = FloatMath.sqrt(
				event.values[0]*event.values[0]+
				event.values[1]*event.values[1]+
				event.values[2]*event.values[2]);
		
		if (norm > 1)	// normally earth magnetic field is 25-65 uTesla, but some sensors are way-off in magnitude, setting limit as 1 uTesla here
		{				    			
	    	for (int i=0; i<3; ++i)
	    		event.values[i] /= norm;
	    	...
	    	if (magneticDataValid > 0)
	    	{
		    	...
		    	for (int i=0; i<3; ++i)
		    	{
		    		...
		    		float mag_old = this.data_magnetic[i]*this.data_magnetic[i]+this.data_magnetic[j]*this.data_magnetic[j];
		    		
		    		if ((mag_old > 0.01) && (mag_new > 0.01))	// note the absence of sqrt for these
		    		{
	...
	*/
	...
	private float fuseAngles(float nice, float dirty, final float compensation, float filtCf)
	{
		...
		if (nice < -Math.PI) nice += 2*Math.PI;
		if (nice > Math.PI) nice -= 2*Math.PI;
	...
	private static void getRotationVectorFromGyro(
			final float[] gyroValues, final float[] deltaRotationVector, final float timeFactor)
	{
		float[] normValues = new float[3];
		
		// Calculate the angular speed of the sample
		float omegaMagnitude =	FloatMath.sqrt(
