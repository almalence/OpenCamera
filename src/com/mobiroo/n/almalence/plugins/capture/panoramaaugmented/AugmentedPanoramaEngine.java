...
package com.mobiroo.n.almalence.plugins.capture.panoramaaugmented;
...
/* <!-- +++
import com.mobiroo.n.almalence.opencam_plus.MainScreen;
+++ --> */
// <!-- -+-
import com.mobiroo.n.almalence.opencam.MainScreen;
//-+- -->

import com.mobiroo.n.almalence.util.ImageConversion;
import com.mobiroo.n.almalence.util.Util;

import com.mobiroo.n.almalence.plugins.capture.panoramaaugmented.AugmentedRotationListener.AugmentedRotationReceiver;
...
public class AugmentedPanoramaEngine implements Renderer, AugmentedRotationReceiver
{
	...
	private static Vector3d rotateVectorAroundAxis(float angle, Vector3d axis, Vector3d vector)
	{
	    ...
	    rotationMatrix[0][1] = (float)(axis.x * axis.y * (1 - Math.cos(angle)) - axis.z * Math.sqrt(L) * Math.sin(angle)) / L;
	    rotationMatrix[0][2] = (float)(axis.x * axis.z * (1 - Math.cos(angle)) + axis.y * Math.sqrt(L) * Math.sin(angle)) / L;
	 
	    rotationMatrix[1][0] = (float)(axis.x * axis.y * (1 - Math.cos(angle)) + axis.z * Math.sqrt(L) * Math.sin(angle)) / L;
	    rotationMatrix[1][1] = (float)(v2 + (u2 + w2) * Math.cos(angle)) / L;
	    rotationMatrix[1][2] = (float)(axis.y * axis.z * (1 - Math.cos(angle)) - axis.x * Math.sqrt(L) * Math.sin(angle)) / L;
	 
	    rotationMatrix[2][0] = (float)(axis.x * axis.z * (1 - Math.cos(angle)) - axis.y * Math.sqrt(L) * Math.sin(angle)) / L;
	    rotationMatrix[2][1] = (float)(axis.y * axis.z * (1 - Math.cos(angle)) + axis.x * Math.sqrt(L) * Math.sin(angle)) / L;
	...
	// initialTransform is used outside to convert world coordinates into device coordinates
	...
	public int getPictureTakingState(final boolean autoFocus)
	{
		...
		synchronized (this.stateSynch)
		{
			...
			// if picture will be taken right away, to allow user to stabilize camera 
			if (!autoFocus)
			{
	...
	public void ViewportCreationTime()
	{
		bCreateViewportNow = true;
	}
	...
	public void onSurfaceChanged(GL10 gl, int width, int height)
	{
		...
		bViewportCreated = false;
		
		iSurfaceWidth = width;
		iSurfaceHeight = height;
		gl10 = gl;
	}
	...
	{
		if(bCreateViewportNow && (!bViewportCreated))
		{
			...
			bCreateViewportNow = false;
			bViewportCreated = true;
		}
	...
	private class AugmentedFrameTarget
	{
		...
		private final byte[] colors = new byte[]
		{
	    	0, 0, -128
	    };
		...
		{	
			...
			if (AugmentedPanoramaEngine.this.state == STATE_TAKINGPICTURE)
			{
				synchronized (this.colorBuffer)
				{
					...
					//*
					part = Math.min((System.currentTimeMillis() - AugmentedPanoramaEngine.this.capture_time) / (float)FRAME_WHITE_FADE_IN, 1.0f);
		...
		public float distance()
		{
			...
			{
				dpos = (float)Math.sqrt(Util.mathSquare(this.position.x - AugmentedPanoramaEngine.this.currentVector.x)
				...
				drot = (float)Math.sqrt(Util.mathSquare(AugmentedPanoramaEngine.this.initialTopVector.x - AugmentedPanoramaEngine.this.topVector.x)
						+ Util.mathSquare(AugmentedPanoramaEngine.this.initialTopVector.y - AugmentedPanoramaEngine.this.topVector.y)
		...
		public void move(final int shift)
		{
			...
			this.position.set(rotateVectorAroundAxis(
					(float)Math.toRadians(this.angle), 
