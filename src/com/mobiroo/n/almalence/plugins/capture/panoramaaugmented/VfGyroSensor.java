...
public class VfGyroSensor implements Closeable, Handler.Callback
{
    ...
    public String getName() { return "ViewFinder Gyro"; }

    public String getVendor() { return "Almalence"; }

    public int getType() { return TYPE_VF_GYROSCOPE; }

    public int getVersion() { return 1; }
    public float getMaximumRange() { return 2*(float)Math.PI; }// full circle in a second

    public float getResolution() { return 1e-5f; }

    public float getPower() { return 0; }

    public int getMinDelay() { return 16000; }	// ~ 60 fps
    ...
    public VfGyroSensor(SensorEventListener listener)
	{
		...
		try
		{
			EventConstructor = SensorEvent.class.getDeclaredConstructor(int.class); // Parameter is the type of params in the constructor (e.g. String)  
    ...
    public void NewData(byte[] data)
    {
    	...
    	{
			// For smoother GUI: if frame is still in processing - pass the previous values,
			// but take this (timestamp, values) into account for the next calculation
			// Simpler way: if not finished with previous frame - do not accept the new one
    	...
    	// To prevent pressure on UI part - run gyro processing in separate thread
			...
			public void run()
			{
		    	synchronized(mThiz)
		    	{
			    	...
			    	synchronized(sensorEventPrev)
			    	{
			    		...
			    		// if there were blank runs - correct for accumulated error
