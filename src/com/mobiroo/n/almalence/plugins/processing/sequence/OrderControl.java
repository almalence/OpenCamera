		...
		if (this.indexes != null)
		{
	        ...
	        for (int i = 0; i < this.indexes.length; i++)
	        {
	        	if (this.indexes[i] != this.touchedIndex)
	        	{
	        		...
	        		final float imageHeight = Math.min(this.height, this.images[actualIndex].getHeight() * imageWidth / (float)this.images[actualIndex].getWidth());
	        		...
	        		canvas.drawRect(
	        				this.outRect.left + BORDER_SIZE / 2.0f,
	        				this.outRect.top + BORDER_SIZE / 2.0f,
	        				this.outRect.right - BORDER_SIZE / 2.0f,
	        				this.outRect.bottom - BORDER_SIZE / 2.0f,
	        				this.paint);
	        	}
	        	else
	        	{
	        ...
	        if (touched >= 0)
	        {
        		...
        		final float imageHeight = Math.min(this.height, this.images[this.touchedIndex].getHeight() * imageWidth / (float)this.images[this.touchedIndex].getWidth());
		...
		case MotionEvent.ACTION_MOVE:
			{
				...
				for (int i = 0; i < Math.abs(shift); i++)
				{
					...
					final int n1 = Math.min(this.indexes.length - 1, Math.max(0, pos + (i + 1) * (int)Math.signum(shift)));
