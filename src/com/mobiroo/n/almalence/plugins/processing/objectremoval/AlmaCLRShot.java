...
package com.mobiroo.n.almalence.plugins.processing.objectremoval;
...
import com.mobiroo.n.almalence.SwapHeap;

import com.mobiroo.n.almalence.util.Size;
...
public class AlmaCLRShot
{
	...
	public boolean initialize(Size previewSize, int angle, int baseFrame, int sensitivity, int minSize, int ghosting, OnProcessingListener listener) throws Exception {
		...
		mAngle = angle;
		mLayoutSize = new Size(mInputFrameSize.getWidth() / IMAGE_TO_LAYOUT, mInputFrameSize.getHeight() / IMAGE_TO_LAYOUT);
	...
	private Rect rotateObjRect(Rect rect) {
		...
		switch(mAngle) {
		case 90:
			newRect = new Rect((mInputFrameSize.getHeight() - rect.bottom), rect.left, (mInputFrameSize.getHeight() - rect.top), rect.right);
			break;
		case 180:
			newRect = new Rect(mInputFrameSize.getWidth() - rect.right, 
					mInputFrameSize.getHeight() - rect.bottom, 
					mInputFrameSize.getWidth() - rect.left, 
					mInputFrameSize.getHeight() - rect.top);
			break;
		case 270:
	...
	private Rect rotateRect(Rect rect) {
		...
		switch(mAngle) {
		case 90:
			newRect = new Rect((mPreviewSize.getHeight() - rect.bottom), rect.left, (mPreviewSize.getHeight() - rect.top), rect.right);
			break;
		case 180:
			newRect = new Rect(mPreviewSize.getWidth() - rect.right, 
					mPreviewSize.getHeight() - rect.bottom, 
					mPreviewSize.getWidth() - rect.left, 
					mPreviewSize.getHeight() - rect.top);
			break;
		case 270:
	...
	synchronized public ObjectInfo[] getObjectInfoList() {
		...
		for (int i = 0;i < mTotalObj;i++) {
			...
			Rect orgRect = new Rect(mBoarderRect[i].left * IMAGE_TO_LAYOUT, 
					mBoarderRect[i].top * IMAGE_TO_LAYOUT, 
					mBoarderRect[i].right * IMAGE_TO_LAYOUT, 
					mBoarderRect[i].bottom * IMAGE_TO_LAYOUT);
			Rect PreviewRect = new Rect(Math.round(mBoarderRect[i].left * ratio),
					Math.round(mBoarderRect[i].top * ratio),
					Math.round(mBoarderRect[i].right * ratio),
					Math.round(mBoarderRect[i].bottom * ratio));
	...
	synchronized public ObjBorderInfo[] getObjBorderBitmap(Paint paint) {
		...
		for (int i = 0;i < mTotalObj;i++) {
			...
			Rect PreviewRect = new Rect(Math.round(mBoarderRect[i].left * ratio -5),
										Math.round(mBoarderRect[i].top * ratio - 5),
										Math.round(mBoarderRect[i].right * ratio + 5),
										Math.round(mBoarderRect[i].bottom * ratio + 5));
			
			Rect LayoutRect = new Rect(Math.round(mBoarderRect[i].left * ratio -5),
					Math.round(mBoarderRect[i].top * ratio -5),
					Math.round(mBoarderRect[i].right * ratio + 5),
					Math.round(mBoarderRect[i].bottom * ratio + 5));
					
			PreviewRect.setIntersect(PreviewRect,  new Rect(0, 0, mPreviewSize.getWidth()-1, mPreviewSize.getHeight()-1));
			LayoutRect.setIntersect(LayoutRect,  new Rect(0, 0, mPreviewSize.getWidth()-1, mPreviewSize.getHeight()-1));
			...
			mObjBorderInfo[i].mThumb = rotateBitmap(getObjBorderSource(i+1, paint, LayoutRect), LayoutRect.width(), LayoutRect.height(), mAngle);
		}
		return mObjBorderInfo;
	}
	...
	synchronized private void removeProcessing(byte[] layout) {
    	...
    	long start = System.currentTimeMillis();
		mOutNV21 = MovObjProcess(mNumOfFrame, mInputFrameSize, mSensitivity, mMinSize, mBaseArea, mCrop, layout, mGhosting, IMAGE_TO_LAYOUT);
    	Log.d(TAG, "MovObjProcess() elapsed time = " + (System.currentTimeMillis() - start));
    	Log.d(TAG, "mCrop[0] = " + mCrop[0] + " mCrop[1] = " + mCrop[1] + " mCrop[2] = " + mCrop[2] + " mCrop[3] = " + mCrop[3]);
	...
	private Bitmap getObjectBitmap(int objectIndex, Rect rect) {
		...
		int[] buffer = null;
		if ((rect.width() / ratio) < 1 || (rect.height() / ratio) < 1 || rect.left < 0 || rect.right < 0 || rect.top < 0 || rect.bottom < 0) {
			return null;
		}

		// adjust object rectangle to match covered area in base frame
		Rect rect_adj = new Rect(
				rect.left*mBaseArea[2]/mInputFrameSize.getWidth()+mBaseArea[0],
				rect.top*mBaseArea[3]/mInputFrameSize.getHeight()+mBaseArea[1],
				rect.left*mBaseArea[2]/mInputFrameSize.getWidth()+mBaseArea[0] + rect.width()*mBaseArea[2]/mInputFrameSize.getWidth(),
				rect.top*mBaseArea[3]/mInputFrameSize.getHeight()+mBaseArea[1] + rect.height()*mBaseArea[3]/mInputFrameSize.getHeight()
				);

		Size size = new Size((int)((float)rect.width() / (float)ratio), (int)((float)rect.height() / (float)ratio));
