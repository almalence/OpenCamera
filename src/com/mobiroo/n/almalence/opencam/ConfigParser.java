...
Parse main configuration - mode.xml, located in assets directory.
mode.xml contains information about default plugin for first run and modes. 

Each mode is a set of plugins of different type. Plugin order in set is not important.

All tags are processed in readTag function.

config example
<mode id="test" name="Test">
 		<icon		id="icon"/>
	    <vf 		id="com.opencam.testvf"/>    
	    <capture 	id="com.opencam.testcapture" />    
	    <processing	id="com.opencam.testprocessing"/>  
		<filter		id="com.opencam.testfilter"/>  
	    <export		id="com.opencam.testexport"/>
		<sku		id="com_opencam_modename"/>
		<howtotext  id="text howto use mode"/>
	</mode> 

each tag has to be in the same format:
<tag_name		id="value"/>

in case of other tag format will be necessary to add processing function in readTag for new tag
***/
