...
public class Seamless
{
	...
	public void addJPEGInputFrames(List<byte[]> inputFrame, Size size, Size fd_size, boolean needRotation, boolean cameraMirrored, int rotationDegree)  throws Exception {
		...
		int error = 0;
		error = AlmaShotSeamless.ConvertAndDetectFacesFromJpegs(PointOfJpegData, LengthOfJpegData, mNumOfFrame, size.getWidth(), size.getHeight(), fd_size.getWidth(), fd_size.getHeight(), needRotation, cameraMirrored, rotationDegree);
	...
	public void addYUVInputFrames(List<Integer> inputFrame, Size size, Size fd_size, boolean needRotation, boolean cameraMirrored, int rotationDegree)  throws Exception {
		mNumOfFrame = inputFrame.size();
		if(rotationDegree != 0 && rotationDegree != 180)
			mInputFrameSize = new Size(size.getHeight(), size.getWidth());
        else
		...
		long start = System.currentTimeMillis();
		int error = 0;
		error = AlmaShotSeamless.DetectFacesFromYUVs(PointOfYUVData, LengthOfYUVData, mNumOfFrame, size.getWidth(), size.getHeight(), fd_size.getWidth(), fd_size.getHeight(), needRotation, cameraMirrored, rotationDegree);
	...
	public boolean initialize(int baseFrameIndex, ArrayList<ArrayList<Rect>> faceInfoList, Size preview) throws Exception {
		if (mNumOfFrame == 0) {
	...
	public Bitmap getPreviewBitmap() {
		...
		mBitmap.setPixels(ARGBBuffer, 0, mPreviewSize.getWidth(), 0, 0, mPreviewSize.getWidth(), mPreviewSize.getHeight());		
		return mBitmap;
	}
	...
	public byte[] processingSaveData() {
        ...
        {
			mCrop = new int[5];
			mOutNV21 = AlmaShotSeamless.RealView(mInputFrameSize.getWidth(), mInputFrameSize.getHeight(), mCrop, mLayout);
	...
	public byte[] getInputFrameBuffer(int index) {
		if (AlmaShotSeamless.getInputFrame(index) == 0) {
			...
			return SwapHeap.CopyFromHeap(AlmaShotSeamless.getInputFrame(index), mInputFrameSize.getWidth() * mInputFrameSize.getHeight() * 3 / 2);
		}
	}
	...
	private void makePreview(List<Rect> faceRect) {
		...
		ARGBBuffer = AlmaShotSeamless.Preview(mBasebaseFrameIndex, mInputFrameSize.getWidth(), mInputFrameSize.getHeight(), mPreviewSize.getWidth(),
				mPreviewSize.getHeight(), mClonedLayout);
		mClonedLayout = null;
		return;
	}
		...
		for (Rect rect : faceRect) {
			...
			if (isFarDistance(rect, dst) == true) {
				newRect = new Rect((int)(dst.left / IMAGE_TO_LAYOUT),
						...
						(int)(dst.bottom/ IMAGE_TO_LAYOUT));
			} else {
				newRect = new Rect((int)(rect.left / IMAGE_TO_LAYOUT),
						(int)(rect.top/ IMAGE_TO_LAYOUT), 
						(int)(rect.right/ IMAGE_TO_LAYOUT), 
						(int)(rect.bottom/ IMAGE_TO_LAYOUT));
			}
