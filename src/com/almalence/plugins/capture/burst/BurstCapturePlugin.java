...
public class BurstCapturePlugin extends PluginCapture
{
	...
	public void takePicture()
	{
		...
		if (captureRAW)
		{
			...
			for(int i = idx; i < IMAGE_AMOUNT_VALUES.size(); i++)
			{
				...
				if(checkFreeMemory(imageAmountChecked))
				{
					...
					CameraController.captureImagesWithParams(imageAmount, CameraController.RAW, pause, null, null, null, 0, true,
							true);
					return;
				}
			}
		...
		CameraController.captureImagesWithParams(imageAmount, CameraController.JPEG, pause, null, null, null, 0, true,
				true);
	}
